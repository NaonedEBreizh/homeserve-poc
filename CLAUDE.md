...
- Seuils de non-rentabilité solaire (P(e)) : facture < 60 €/mois, surface < 70 m², résidence secondaire, en construction → pas d'arrêt. **D57** : écran d'orientation intermédiaire (B18o, « Avant de réserver, un point d'honnêteté ») avec trois choix — réserver quand même (note technicien `regle:<code>`), être rappelé, découvrir la PAC si le projet est solaire seul. Idem hors zone (B14z) et agence saturée (B19c) : le prospect peut toujours réserver.
...
- Surbooking (D30, révisé par D57) : < 4 créneaux libres sur 10 jours → écran « forte demande », au choix voir le calendrier complet ou être rappelé. CP de test `99999` → agence `TEST` surbookée.
