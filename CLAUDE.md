...
- Seuils de non-rentabilité solaire (P(e)) : facture < 60 €/mois, surface < 70 m², résidence secondaire, en construction → pas d'arrêt. **D57** : écran d'orientation intermédiaire (B18o, « Avant de réserver, un point d'honnêteté ») — réserver quand même (note technicien `regle:<code>`) ou découvrir la PAC si le projet est solaire seul. Idem hors zone (B14z) et agence saturée (B19c) : le prospect peut toujours réserver.
- Sorties immédiates, seules vraies exclusions : local pro (S1), appartement (S2), locataire (S3 → assistance 6 €/mois), panneaux d'un autre installateur (S4c), toit impossible (S5 → PAC). URLs réelles dans `content/fr-fr.json`.
...
- Surbooking (D30, révisé par D57 et D58) : < 4 créneaux libres sur 10 jours → écran « forte demande », qui n'ouvre que sur les créneaux plus lointains. CP de test `99999` → agence `TEST` surbookée.
...
- **D58** : le prototype ne propose **aucune** demande de rappel. Plus de route `/sortie/R1`, plus de choix « Être rappelé » sur les écrans d'orientation, plus d'événement `callback_requested`. Quand aucun créneau ne convient, le calendrier étend son horizon (14 → 28 jours, `agences.json`) ; hors horaires, le lien d'en-tête annonce les horaires et renvoie à la prise de rendez-vous (`booking_out_of_hours`).
